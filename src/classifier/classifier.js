...
// llama3.2:1b was tried first for CPU-only, decade-old hardware, but under
// JSON-schema-constrained decoding it reliably wrote a correct category and
// reason (e.g. "insult" / "contains threats directed at me") and then set
// flagged: false anyway — the verdict field didn't track its own reasoning.
// llama3.2:3b got every hand-tested case right and stayed consistent, so
// it's the default despite being heavier; only drop to 1b if the host
// machine truly can't keep up, and re-verify carefully against known cases
// if you do (see npm run classifier:test).
